...
if (isWujie && window.$wujie) {
  ...
  if (useStandardRouteSync) {
    import('@k8s-agent/shared/core/route-sync.js').then(({ RouteSync }) => {
      const routeSync = new RouteSync('agent-app', window.$wujie.bus, router)
      routeSync.setupListener()
      window.__ROUTE_SYNC__ = routeSync
      console.log('[Agent App] ✅ RouteSync listener set up')
    }).catch(() => {
      window.$wujie.bus.$on('agent-app-route-change', (subPath) => {
        if (router.currentRoute.value.path !== subPath) router.push(subPath)
      })
    })
  } else {
    window.$wujie.bus.$on('agent-app-route-change', (subPath) => {
      console.log('[Agent App] Received route change from main app:', subPath)
...
// 始终挂载到 #agent-app（Wujie 会加载完整的 HTML）
app.mount('#agent-app')
