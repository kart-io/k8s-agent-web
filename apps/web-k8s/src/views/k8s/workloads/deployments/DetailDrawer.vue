...
const labels = computed(() => {
  if (!props.deployment?.metadata.labels) return [];
  return Object.entries(props.deployment.metadata.labels).map(
    ([key, value]) => ({
      key,
      value,
    }),
...
const annotations = computed(() => {
  if (!props.deployment?.metadata.annotations) return [];
  return Object.entries(props.deployment.metadata.annotations).map(
    ([key, value]) => ({
      key,
      value,
    }),
...
const selectors = computed(() => {
  if (!props.deployment?.spec.selector.matchLabels) return [];
  return Object.entries(props.deployment.spec.selector.matchLabels).map(
    ([key, value]) => ({
      key,
      value,
    }),
...
/**
 * 生成 YAML 格式的 Deployment 配置
 */
const deploymentYaml = computed(() => {
  if (!props.deployment) return '';

  // 构建完整的 Deployment YAML 对象
  const yamlObj = {
    apiVersion: props.deployment.apiVersion,
    kind: props.deployment.kind,
    metadata: props.deployment.metadata,
    spec: props.deployment.spec,
...
function downloadYaml() {
  ...
  const blob = new Blob([deploymentYaml.value], { type: 'text/yaml' });
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url;
  a.download = `${props.deployment.metadata.name}.yaml`;
  a.click();
...
<template>
  <Drawer
    :open="visible"
    :title="`Deployment 详情 - ${deployment?.metadata.name || ''}`"
    width="80%"
    placement="right"
    @close="handleClose"
  >
    <div v-if="deployment" class="detail-drawer-content">
      <Tabs v-model:active-key="activeTab" type="card" class="detail-tabs">
        ...
        <Tabs.TabPane key="basic" tab="基本信息">
          <Descriptions :column="2" bordered size="small">
            ...
            <Descriptions.Item label="可用副本">
              {{ deployment.status?.availableReplicas ?? 0 }}
            </Descriptions.Item>
            <Descriptions.Item label="更新副本">
            ...
            <Descriptions.Item label="创建时间" :span="2">
              {{ formatDateTime(deployment.metadata.creationTimestamp) }}
            </Descriptions.Item>
          </Descriptions>
        </Tabs.TabPane>
        ...
        <Tabs.TabPane key="containers" tab="容器规格">
          <Table
            ...
            :data-source="deployment.spec.template.spec.containers"
            :pagination="false"
            size="small"
        ...
        <Tabs.TabPane key="conditions" tab="条件">
          <Table
            ...
            :data-source="deployment.status?.conditions || []"
            :pagination="false"
            size="small"
    ...
    <div v-else class="empty-text">未选择 Deployment</div>
  </Drawer>
</template>
